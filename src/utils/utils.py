...
def show_all_birthdays(book: AddressBook):
    birthdays = [str(record) for record in book.values() if record.birthday]
    if not birthdays:
...
def search_by_date_birthday(args, book: AddressBook):
    ...
    results = [str(record) for record in book.values() if record.birthday and record.birthday.value == date]
    if not results:
